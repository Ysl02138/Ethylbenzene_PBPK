...
str <- which(colnames(Rat_mcmc) == "M_lnPBC(1)")
end <- which(colnames(Rat_mcmc) == "M_lnKpgaC(1)")
...
x1 <- melt(mSI) |> `colnames<-`(c("parameter", "variable", "index")) |>
  mutate(order = "main effect", specice = "Rat", exposure = "75 ppm")
x2 <- melt(iSI) |> `colnames<-`(c("parameter", "variable", "index")) |>
  mutate(order = "interaction", specice = "Rat", exposure = "75 ppm")
...
x3 <- melt(mSI) |> `colnames<-`(c("parameter", "variable", "index")) |>
  mutate(order = "main effect", specice = "Rat", exposure = "750 ppm")
x4 <- melt(iSI) |> `colnames<-`(c("parameter", "variable", "index")) |>
  mutate(order = "interaction", specice = "Rat", exposure = "750 ppm")
