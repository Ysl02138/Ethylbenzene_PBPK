...
str <- which(colnames(Human_mcmc) == "M_lnPBC(1)")
end <- which(colnames(Human_mcmc) == "M_lnKpgaC(1)")
...
params <- c("lnPBC", "lnPFatC", "lnPLivC", "lnPRptC", 
  ...
  "lnKM2C", "lnVmaxLungC", "lnKMLungC",
  "lnFmeteC", "lnKmaC", "lnKpgaC")
q <- rep("qunif", 15)
...
x3 <- melt(mSI) |> `colnames<-`(c("parameter", "variable", "index")) |>
  mutate(order = "main effect", specice = "Human", exposure = "750 ppm")
x4 <- melt(iSI) |> `colnames<-`(c("parameter", "variable", "index")) |>
  mutate(order = "interaction", specice = "Human", exposure = "750 ppm")
...
ggplot(x_total, aes(x = parameter, y = index, fill=order)) +
  ...
  set_theme
ggsave(file = "plots/suppl/Supplemental_Figure_S12_Human EB GSA.jpg", height = 12, width = 20, dpi = 600)
dev.off()  
